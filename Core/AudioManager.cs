    ...
    public class AudioManager : IAudioManager, IDisposable
    {
        ...
        public async Task<List<AudioSession>> GetAllAudioSessionsAsync()
        {
            return await Task.Run(() =>
            {
                lock (_lock)
                {
                    return GetAllAudioSessionsInternal();
                }
            });
        }
        ...
        public async Task<bool> SetProcessVolumeAsync(int processId, float volume)
        {
            return await Task.Run(() =>
            {
                lock (_lock)
                {
                    return SetProcessVolumeInternal(processId, volume);
                }
            });
        }
        ...
        public async Task<bool> MuteProcessAsync(int processId, bool mute)
        {
            return await Task.Run(() =>
            {
                lock (_lock)
                {
                    return MuteProcessInternal(processId, mute);
                }
            });
        }
        ...
        private List<AudioSession> GetAllAudioSessionsInternal()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < sessionCount; i++)
                {
                    ...
                    {
                        _logger.LogWarning(ex, "Error processing audio session {SessionIndex}", i);
                    }
                }
